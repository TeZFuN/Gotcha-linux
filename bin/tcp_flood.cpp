// g++ -O2 -pthread -march=x86-64 -mtune=generic -std=c++11 tcp_flood.cpp -o NPtcpT -lpcap
#include <pcap.h>
#include <unistd.h>
#include <net/if.h>
#include <sys/ioctl.h>
...
public:
    ...
    PrebuiltPacket(uint32_t src_ip, const uint8_t* src_mac,
        ...
        size_t min_size = sizeof(my_eth_header) + sizeof(my_ip_header) + sizeof(my_tcp_header);
        ...
        ip->tot_len = htons(sizeof(my_ip_header) + sizeof(my_tcp_header) + payload);
        ip->id = 0; ip->frag_off = 0; ip->ttl = 64; ip->protocol = 6; // TCP
        ip->check = 0;
        ip->saddr = src_ip; ip->daddr = dst_ip;

        my_tcp_header* tcp = (my_tcp_header*)(buffer.data() + sizeof(my_eth_header) + sizeof(my_ip_header));
        tcp->source = 0;
        ...
        if (payload) {
            uint8_t* pay = buffer.data() + sizeof(my_eth_header) + sizeof(my_ip_header) + sizeof(my_tcp_header);
            memset(pay, 0, payload);
        }

        src_ip_offset = sizeof(my_eth_header) + offsetof(my_ip_header, saddr);
        src_port_offset = sizeof(my_eth_header) + sizeof(my_ip_header) + offsetof(my_tcp_header, source);
        seq_offset = sizeof(my_eth_header) + sizeof(my_ip_header) + offsetof(my_tcp_header, seq);
        tcp_offset = sizeof(my_eth_header) + sizeof(my_ip_header);
                   }
                   ...
                   void set_seq(uint32_t seq) {
                       *(uint32_t*)(data() + seq_offset) = htonl(seq);
                   }
                   ...
                   void recalc_checksum() {
                       ...
                       size_t tcp_len = buffer.size() - sizeof(my_eth_header) - sizeof(my_ip_header);
                       if (tcp_len > 65535) tcp_len = 0;
                       ps.len = htons((uint16_t)tcp_len);

                       my_tcp_header* tcp = (my_tcp_header*)(data() + tcp_offset);
                       tcp->check = 0;

                       size_t total = sizeof(ps) + tcp_len;
                       std::vector<uint8_t> stack(total);
                       memcpy(stack.data(), &ps, sizeof(ps));
                       memcpy(stack.data() + sizeof(ps), tcp, tcp_len);
                       tcp->check = calc_checksum((uint16_t*)stack.data(), (int)total);
                   }
};
